...
from mbart_model import MBartModel, MBartPreTrainedModel, MBartEncoder, MBartDecoder
...
class MBartModelMultHeads(MBartPreTrainedModel):
    def __init__(self, config: MBartConfig):
        ...
        self.decoder1 = MBartDecoder(config, self.shared)
        self.decoder2 = MBartDecoder(config, self.shared)
...
class ConditionalGenerationCustomMBartMultHeads(GenerationMixinCustom, MBartPreTrainedModel):
    base_model_prefix = "model"
    authorized_missing_keys = [r"final_logits_bias", r"encoder\.version", r"decoder\.version"]

    def __init__(self, config: MBartConfig):
        super().__init__(config)
        base_model = MBartModelMultHeads(config)
        self.model = base_model
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))

    def initialize_correct_weights(self, config: MBartConfig, num_decoder_layers_shared=6):
