...
MODEL_CLASSES = {"mbart_mult_heads_2": (MBartConfig,
                                     multi_head_utils.ConditionalGenerationCustomMBartMultHeads,
                                     MBartTokenizer),
                 }
...
class MBartModelCombined(GenerationMixinCustomCombined, nn.Module):
...
def load_model(path):
    ...
    config_class, model_class = MBartConfig, multi_head_utils.ConditionalGenerationCustomMBartMultHeads
...
def main():
    ...
    tokenizer = MBartTokenizer.from_pretrained("facebook/mbart-large-cc25", src_lang="ru_RU", tgt_lang="ru_RU")

    model = MBartModelCombined(model1, model2, config)
